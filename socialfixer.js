...
.sfx_edit_buf_button .sfx_selected {
  outline: 2px solid red;
}
.sfx_edit_buf_toggle {
  font-weight: normal;
...
X.ready('comment_button', function () {
  ...
  FX.add_option('comment_button', {"title": title, "order": 1, "description": "Use Enter to add a new line instead of submitting comments & replies.", "default": false});
  FX.add_option('comment_button_msgs', {"title": title, "order": 2, "description": "Use Enter to add a new line instead of submitting chat / messages.", "default": false});
  FX.add_option('comment_button_ctrl', {"title": title, "order": 3, "description": "Use Ctrl+Enter to submit comments, replies & chat / messages.", "default": false});
  FX.add_option('comment_button_emergency', {"title": title, "order": 4, "description": "Use alternate method (no Submit buttons).", "default": false});
...
(Facebook's normal behavior)` :
                                "Fix Enter should work, but there will be no Submit button";
        alert(`Message from Social Fixer: it looks like 'Fix Enter' is failing.

Please ${proposed_solution}, and watch
https://facebook.com/groups/SocialFixerUserSupport
for announcements.
...
${proposed_limitation}.`);
        ...
        document.documentElement.setAttribute("sfx_fix_comments_failing", "true");
        document.querySelector('.sfx_comment_button').style.display = "none";
        document.querySelector('.sfx_comment_button_msg').textContent = "Press 'Enter' to submit";
      }, 250);
    ...
    X.capture(window, 'keydown', function (e) {
      ...
      if (document.documentElement.getAttribute('sfx_fix_comments_failing')) {
        return;
      }
      ...
      if (FX.option('comment_button_emergency')) {
        if (e.keyCode == 13) {
          dispatch_enter_event(e, $target, !(FX.option('comment_button_ctrl') && e.ctrlKey));
        }
        // else let FB handle it normally
        return;
      }

      // ==> If this isn't an editable field
      if ('true' != $target.attr('contenteditable') && !$target.attr('data-reactid')) {
        return;
      }

      // ==> If this isn't the sort of comment container we know about
      // (.textBoxContainer appears to be obsolete 2018-11-01)
      var $container = $target.closest('.textBoxContainer,.UFICommentContainer,[data-testid*="CommentsList"] ._42ef > div,.fbNubFlyoutFooter');
      // Separate because some of the above would find a wrong ._5rpu
      $container.length || ($container = $target.closest('._5rpu'));
      if (!$container.length) {
        return;
      }
      ...
      var comment_type = /ufi/i.test($target.attr('data-testid')) ? 'comment' : 'message';
      ...
      if (comment_type == 'comment') {
        $target[0].tabIndex = 9900 + 2 * this_comment_id;

        var $note_container = $container.parent();
        if (!$note_container.find('.sfx_comment_button_present').length) {
          var html = FX.oneLineLtrim(`
            <input v-if="comment_button" class="sfx_comment_button" type="button" value="Submit ${/reply/.test($target.attr('data-testid')) ? 'Reply' : 'Comment'}" style="cursor:pointer;" tabIndex="${9901 + 2 * this_comment_id}" @click="dispatch_enter_event($event, Xtarget, false)">
            <span v-if="comment_button && comment_button_ctrl" class="sfx_comment_button_msg">(Ctrl+Enter also submits)</span>
            <span v-if="!comment_button && comment_button_hint" class="sfx_comment_button_msg">
              Social Fixer can prevent Enter from submitting comments & replies!
            ...
            <span class="sfx_comment_button_present">
            </span>
          `);
...
X.ready( 'comment_navigator', function() {
    ...
    FX.add_option('navigator_order', {"title": 'Comment Navigator', "description": "Try to set comment order to (e.g. 'All' or 'New')", "type": "text", "default": ""});
    ...
    var expander_selector = [
        '[sfx_post]:not([sfx_clicked]) ._3scs>._p',                   /* UFI1: comment order */
        '[sfx_post]:not([sfx_clicked]) ._p[data-testid="UFI2ViewOptionsSelector/link"]', /* UFI2: comment order */
        ':not(.UFIAddCommentLink) > .UFICommentLink',                 /* UFI1: Xyz replied . N Replies */
        'a[data-testid="UFI2CommentsPagerRenderer/pager_depth_1"]',   /* UFI2: N Replies */
        '.UFIPagerLink',                                              /* UFI1: View N more comments */
        'a[data-testid="UFI2CommentsPagerRenderer/pager_depth_0"]',   /* UFI2: View N more comments */
        'a._ipm[href*="comment"]:not([sfx_clicked])',                 /* UFI1: N Comments: search results */
        'a[data-testid="UFI2CommentsCount/root"]:not([sfx_clicked])', /* UFI2: N Comments */
        'a._5v47.fss[role="button"]',                                 /* UFIx: "See More" on comment/reply */
        'a.see_more_link:not([sfx_clicked])',                         /* UFIx: "See More" on post */
    ].join(',');
    ...
    var scroll_to_expander = function ($post) {
    ...
    var expanders = function ($post) {
        return $post.find(expander_selector).filter(function () {
            // `this` is a single filtered Element object
            var $e = X(this);

            // '._ipm' is the top-of-post '%d Comments' toggle button;
            // some posts arrive with this already opened, others not;
            // opened post has <div class='UFIList'><h6>Comments</h6>;
            // but don't look for 'Comments', which will be localized.
            if (($e.hasClass('_ipm') && $post.find('.UFIList > h6.accessible_elem').length) ||
                ($e.attr('data-testid') == 'UFI2CommentsCount/root' && $post.find('[data-testid^="UFI2CommentsList/root_depth_"]').length)) {
                return false;
            }
            return (!$e.find('*[aria-busy],.UFICollapseIcon').length);
        });
    };
    var post_set_expander_visibility = function (cnd) {
        cnd.show_expand_all = expanders(cnd.post$).length;
        if (!cnd.show_slider) {
            cnd.show_slider = !!cnd.post$.find(has_comments_selector).length;
        }
    };
    var post_expand = function () {
        // `this` is Vue object, including `data` == comment_navigator_data[id];
        // exposed to subfunctions as `cnd`
        var cnd = this;

        var desired_order = FX.option('navigator_order').trim();
        var expand_all = function () {
            var expand_progress_bar = function() {
                var pct = 100 * cnd.limit / expand_limit;
                cnd.post$.find('div.sfx_expander_bar')
                     .css("background", 'linear-gradient(to right, rgba(124,157,189,.3) ' + pct + '%, transparent ' + pct + '%)');
            };
            post_set_expander_visibility(cnd);
            expanders(cnd.post$).each(function (i, o) {
                try {
                    cnd.active = true;
                    expand_progress_bar();
                    var $o = X(o);
                    var extra_delay_ms = 0;
                    ...
                    if (was_clicked_ms && X.now() - was_clicked_ms < refractory_interval_ms) {
                        ...
                        // Only click the order selector if user has set an order
                        // Order selector will be filtered out of subsequent calls
                        // to expanders(), by code there; this is only run once.
                        if ($o.is('._p')) {
                            // Store the order selector's sfx_clicked mark in the
                            // top-level post because FB sometimes rewrite the ._p
                            // part of the DOM when it's clicked.
                            cnd.post$.attr('sfx_clicked', X.now());
                            if (desired_order != '') {
                                X.ui.click($o);
                            }
                        } else {
                        ...
                        if (desired_order != '' && $o.is('._p')) {
                            // Order selector (All)
                            // 0.75s is enough time for the menu to have appeared
                            setTimeout(function () {
                                // Menu is not a child of the button; appears far away in the HTML
                                var order_items = X('.__MenuItem ._54nh ._3scn,.__MenuItem ._54nh ._1ojr');
                                for (var item = 0; item < order_items.length; ++item) {
                                    var $item = X(order_items[item]);
                                    if (!$item.closest('.hidden_elem').length && RegExp(desired_order, 'i').test($item.text())) {
                                        $item.click();
                                        break;
                                    }
                                }
                            }, 0.75 * X.seconds);
                            // 1.75s is enough time for the comment framework to have reloaded
                            extra_delay_ms = (0.75 + 1.75) * X.seconds;
                        }
                        cnd.limit--;
                    }
                    if (cnd.limit > 0) {
                        //console.log("calling again, " + cnd.limit + " / " + cnd.show_expand_all + " to go");
                        setTimeout(function () {
                            expand_all();
                        }, click_interval_ms + extra_delay_ms);
                    }
                    else {
                        expand_progress_bar();
                        post_update_time(cnd);
                        cnd.limit = expand_limit;
                        cnd.active = false;
                    }
            ...
            post_set_expander_visibility(cnd);
            if (cnd.show_expand_all == 0) {
                cnd.active = false;
            }
            expand_progress_bar();
            if (FX.option('navigator_watch') && !cnd.active) {
                setTimeout(function () {
                    post_set_expander_visibility(cnd);
                    scroll_to_expander(cnd.post$);
                }, 0.8 * X.seconds);
            }
        };
        expand_all();
    };
    var post_update_time = function (cnd) {
        cnd.now = X.now();
        if (!cnd.post_time) {
            cnd.post_time = cnd.now;
        }
        var upd_post_time = cnd.post_time;  // reduce Vue data setter calls
        cnd.post$.find('form abbr[data-utime]').each(function (i, o) {
            var ut = (+X(o).attr('data-utime') || 0) * X.seconds;
            if (ut && ut < upd_post_time) {
                upd_post_time = ut;
            }
        });
        if (cnd.post_time != upd_post_time) {
            cnd.post_time = upd_post_time;
        }
        cnd.max = cnd.now - cnd.post_time;
    };
    var post_change = function () {
        ...
        cnd.post$.find('form abbr[data-utime]').each(function (i, o) {
            var $o = X(o);
            var ut = (+$o.attr('data-utime') || 0) * X.seconds;
            if (!ut) {
                return;
            }
            if (ut >= then - cnd.range) {
                $o.css('background-color', 'yellow');
            }
            else {
                $o.css('background-color', '');
    ...
    var post_ago = function () {
        return X.ago(this.now - this.range, this.now, true, true);
    };

    var html = FX.oneLineLtrim(`
        <div v-if="show_expand_all || show_slider" class="sfx_expander" style="border:1px solid #E1E2E3;padding:10px;">
            <div v-if="show_expand_all" class="sfx_link sfx_expander_bar" style="float:right;" @click="expand">
                <span v-if="(active)">Expanding</span>
                <span v-else>Click to expand</span>
                &nbsp;
                <span v-if=(show_expand_all==1)>the </span>
                <span v-else>
                    <span v-if="(show_expand_all>limit)">{{limit}} of </span>
                    <span v-else>all </span>
                    <span>{{show_expand_all}} </span>
                </span>
                hidden item
                <span v-if="(show_expand_all>1)">s</span>
            </div>
            &nbsp;
            <div v-if="show_slider">
                <div>Highlight comments newer than: <b>{{ago()}}</b></div>
                <div>
                    <input v-model="range" type="range" min="0" max="{{max}}" style="width:95%;" @change="change">
                </div>
            </div>
        </div>
    `);
    var methods = {
        // methods called by Vue receive the Vue object as `this`,
        // including  `data` == comment_navigator_data[id]
        "expand": post_expand,
        "change": post_change,
        "ago":    post_ago,
    };
    var has_comments_selector = [
        '.UFIComment',
        '._ipm[href*="comment"]',
        'a[data-testid^="UFI2CommentsCount"]',
    ].join(',');
    var comment_container_selector = [
        'form.commentable_item',
        'form .UFIContainer',
        'form [data-testid="UFI2CommentsList/root_depth_0"]',
        'form.collapsed_comments',
    ].join(',');
    var comment_navigator_data = {};

    X.subscribe(['post/action/commentnavigator/add', 'post/action/commentnavigator/add/always'], function (msg, data) {
        var dom_id = data.id;

        var cnd = comment_navigator_data[dom_id];
        if (!cnd) {
            cnd = comment_navigator_data[dom_id] = {
                // Vue doesn't like '$post'
                "post$": data.dom || X(document.getElementById(dom_id)),
                "range": 0,
                "limit": expand_limit,
                "active": false,
            };
        ...
        if (cnd.show_expand_all || cnd.show_slider) {
            if (!cnd.post$.find('div.sfx_expander').length) {
                var $vue = template(null, html, cnd, methods);
                var comment_container = cnd.post$.find(comment_container_selector).first();
                if (comment_container.length) {
                    comment_container.before($vue.fragment);
                } else {
                    X.support_note('comment_navigator', `no comment_container (id = ${dom_id})`);
                }
            }
            if (! /always/.test(msg)) {
                scroll_to_expander(cnd.post$);
...
X.ready('edit_buffer', function() {
    FX.add_option('edit_buffer', {
        section: 'Experiments',
        title: 'Find Edit',
        description: "If you were editing a post, comment or reply, and can't find it.  Before using this: if you use SFx tabs, select 'All Posts' tab; if you use 'mark as Read', use wrench > Show posts marked 'Read'.",
        type: 'action',
    ...
    X.subscribe('edit_buffer/find', function () {
        ...
        var row_content = function (db) {
            ...
            return (row.length < 80) ? row
                                     : row.slice(0, 77) + '...';
        };

        var dirty_buffers = X('[data-contents] [data-text]:not(:empty)'), db_l = dirty_buffers.length;
			...
			if (FX.option('fix_timestamps')) {
				...
				var fix_timestamps = function (o) {
					...
					o.find('abbr[data-utime][title]').each(function (i, a) {
						...
						if (use_short) {
							var utime = new Date(a.attr('data-utime') * X.seconds);
							if (utime) {
...
X.ready('friend_manager', function() {
    ...
    X.subscribe("friends/options", function(msg,d) {
        ...
        var dialog = FX.oneLineLtrim(`<div id="sfx_friend_dialog" class="sfx_dialog flex-column" style="transition: height .01s;">
...
`);
    ...
    var retrieve_friends = function(cb) {
        ...
        var fb_dtsg = X('[name="fb_dtsg"]').val();
    ...
    var update_jewel_count = function(alerts) {
        ...
        if (alerts.every(function (e) {
            return e.type == 'unfriend';
        })) {
            count = 0 - count;
        }
        ...
        if (el) {
            ...
            $clone.find('span').addClass('hidden_elem').text(count);
            var $container = $c.closest('.jewelButton');
            $container.css('opacity', 1).append($clone);
            setTimeout(function() {
                X('#sfx_friend_jewel_count').find('span').removeClass('hidden_elem');
            }, 1 * X.seconds);
        }
    };
    ...
    var show_alerts = function(ago) {
        ...
        var alerts = find_alerts(friends);
        if (alerts && alerts.length > 0) {
            X.when('#requestsCountValue', function(/* $element */) {
            ...
            FX.on_selector('.requestsJewelLinks', function () {
                if (document.getElementById('sfx_friend_changes')) {
                    return;
                }
    ...
    var render_alerts = function(alerts, ago, show_header, $prependTo) {
        try {
            ...
            var t = FX.oneLineLtrim(`<div id="sfx_friend_changes" style="max-height:300px;overflow:auto;border-bottom:1px solid rgb(221,223,226);">
        ...
        X.subscribe("hide/on", function () {
            ...
            var content = X(FX.oneLineLtrim(`
                    <div class="sfx_hide_bubble">
                        ...
                        <span><input type="button" class="sfx_button sfx_button_done" value="Done Hiding"></span>
            ...
            var show_hideables = function (hideables, warn_server) {
                ...
                hideables.forEach(function (hideable) {
                    ...
                    for (var instance = 0; instance < o.length; instance++) {
                        ...
                        var wrapper = X(`<div data-hover="tooltip" data-tooltip-content="${hider_title(hideable)}" data-tooltip-delay="200" class="sfx_hide_frame" style="width:${w}px;height:${h}px;font-size:${h / 1.5}px;line-height:${h}px;${position}">X</div>`);
...
// Hide hovercards
X.ready('hide_hovercards', function() {
    ...
    FX.on_option('hide_hovercards', function () {
        // This hides the chat sidebar 'hovercards'; they aren't so easy to
        // intercept, so no 'Ctrl' feature.  Working CSS as of 2018-10-17.
        FX.css('div.uiLayer[data-ownerid]>div>div._5v-0._53im { display:none }');
        ...
        X.capture(window, 'mouseover', function (e) {
            ...
            if (e.ctrlKey) {
                ...
                if (xhc) {
                    $t.attr('data-hovercard', xhc).removeAttr('data-x-hovercard');
                }
            }
            else {
                var hc = $t.attr('data-hovercard');
                if (hc && !/\/pubcontent/.test(hc)) {
                    $t.attr('data-x-hovercard', hc).removeAttr('data-hovercard');
                }
            }
        });
    });
        ...
        FX.on_content_loaded(function () {
            ...
            var html = FX.oneLineLtrim(`
                <div id="sfx_post_action_tray">
                    <div v-if="show_mark_read && !read" @click="mark_unmark($event)" class="mark_read_markit" v-tooltip="Mark this post as 'Read', so it doesn't appear in your feed anymore. Shift+Click Marks as 'Read' all posts above here; ${Ctrl}+Click Marks here and below."></div>
                    <div v-if="read" @click="mark_unmark($event)" v-tooltip="Un-Mark this post as 'Read', so it may show up in your feed again.">X</div>
                    <div v-if="!show_mark_read && !read" @click="mark_unmark($event)" class="mark_read_nomark" v-tooltip="Marking this post 'Read' will only hide it in the current session, as it lacks a unique Facebook identifier. Posts like this may be Markable in the future."></div>
                    <div v-if="wrench_items.length>0" id="sfx_mark_read_wrench" class="mark_read_wrench"></div>
                    <div v-if="filters_enabled && filter_items.length>0" id="sfx_mark_read_filter" class="mark_read_filter"></div>
                </div>
            ...
            var hide_read = function ($post) {
                if (!$post.hasClass('sfx_post_read')) {
                    ...
                    if (FX.option('mark_read_display_message')) {
                        var ts = $post.find('abbr.timestamp,abbr.sfx_no_fix_timestamp').first().attr('title');
                        ts = ts ? ts : $post.find('._5ptz').first().attr('title');
                        ts = ts ? ts : $post.find('abbr .timestampContent').first().innerText();
                        ts = ts ? ts : 'unknown';
                        ts = `Click to view 'Read' post (posted: ${ts})`;
            ...
            X.subscribe(["post/add", "post/update"], function (msg, data) {
                ...
                if (msg == "post/add") {
                    ...
                    data.dom.on('mouseenter', function (e) {
                        ...
                        move_action_tray_to_post(data.dom);
                    });
                }
            }, true);
...
var init_wrench = function(disabled) {
	...
	// Check every second to make sure the badge appears
	var check_badge = function() {
		var $badge = X('#sfx_badge');
		if (!$badge.length) {
			position_badge(null, null, false);
			setTimeout(check_badge,1000);
		}
	};
	setTimeout(check_badge,1000);
	...
	actions.add('links', {'html': 'Support Group', 'url': 'https://www.facebook.com/groups/SocialFixerUserSupport/'});
	if (disabled) {
	...
	X(window).on('keydown', function (e) {
...
X.ready( 'notification_popup', function() {
    ...
    FX.on_options_load(function () {
        ...
        X.when('#fbNotificationsFlyout .uiHeaderActions', function ($actions) {
            // Separator
            $actions.prepend(X(`<span role="presentation" aria-hidden="true"> &middot; </span>`));
                ...
                var html = FX.oneLineLtrim(`
                    <div id="sfx_notification_popup_header">
                        <div id="sfx_notification_popup_header_actions">
                            <span class="sfx_link" @click.capture.prevent.stop="mark_all_read" data-hover="tooltip" data-tooltip-position="below" data-tooltip-content="Social Fixer: mark all of these Notifications as read; with ${Ctrl}, also close the popup" data-tooltip-delay=1000>Mark All Read</span>
                            <span>
	...
	FX.on_options_load(function () {
		...
		X.subscribe('menu/options', function (event, event_data) {
			...
			try {
				...
				var dialog = FX.oneLineLtrim(`<div id="sfx_options_dialog" class="sfx_dialog flex-column" style="transition: height .01s;">
	<div id="sfx_options_dialog_header" class="sfx_dialog_title_bar" style="cursor:move;" @click="collapse">
		Social Fixer
					                ...
					                <div class="flex-row-container">
					                    ...
					                    <template v-if="rule.target=='image'">
					                        <div class="stretch" data-tooltip-delay="100" v-tooltip="Match keywords in Facebook's auto-generated description, if they exist. This is not always reliable (yet) due to Facebook's inconsistent labeling.">
					                            <input class="stretch sfx_input" v-model="rule.condition.text" v-bind:disabled="editing_filter.id">
					                        </div>
					                    </template>
					        ...
					        <div class="sfx_options_dialog_filter_actions sfx_options_dialog_panel">
					            ...
					            <div class="flex-row-container" v-for="action in editing_filter.actions">
					                ...
					                <span class="stretch" v-if="action.action=='class'">
					                    <input class="sfx_input" v-model="action.content" size="45" v-bind:disabled="editing_filter.id && !editing_filter.configurable_actions" data-tooltip-delay="100" v-tooltip="Add a class name to the post container. This is useful in conunction with a Display Tweak to customize CSS">
					                </span>
...
`);
				...
				var methods = {
					...
					, "close_filter": function () {
						...
						delete this.editing_filter.new;
					...
					, "add_filter": function () {
						var new_filter = {"match": "ALL", "enabled": true, "stop_on_match": true, "rules": [{"target": "any", "operator": "contains"}], "actions": [{"action": "hide"}], "new": true};
					...
					, "close_tweak": function () {
						delete this.editing_tweak.new;
						this.editing_tweak.updated_on = X.time();
						this.tweaks[this.editing_tweak_index] = X.clone(this.editing_tweak);
						this.tweak_css_on_off(this.editing_tweak_index, this.editing_tweak.enabled);
						this.editing_tweak = null;
						this.action_button = null;
						mark_subscribed_items(data.tweak_subscriptions, tweaks);
					}
					, "add_tweak": function () {
						var new_tweak = {"title": "", "description": "", "enabled": true, "new": true};
...
X.ready('post_filters', function() {
    ...
    var group_hovercard_selector = [
        "._1s31 a[data-hovercard][href^='/groups/']",
        "._5pbw a[data-hovercard][href^='/groups/']",
    ].join(',');
      ...
      FX.on_options_load(function () {
        ...
        var decide_filter_post = function (post, dom_id) {
            ...
            if (FX.context.type != 'groups') {
                var group_hovercard = post.find(group_hovercard_selector).last();
    ...
    var extract = {
        ...
        "group": function (o, data) {
            data.group = null;
            var $el = o.find(group_hovercard_selector);
        ...
        "page": function (o, data) {
            data.page = null;
            var $el = o.find("._5pbw .fwb a[data-hovercard*='/hovercard/page.php']");
            if ($el.length) {
        ...
        "link_url": function (o, data) {
            // 1. <div class="mtm"> encapsulates the post but none of the comment stuff
            // 2. Currently (2018-06-27) posts with links have <a href="https://l.facebook.com/l.php?u=http%3A%2F%2Fsome.where%2Flink-on-site.html%3Fsite-parameters&h=[bunch of FB encoded gibberish]" ... data-lynx-mode="asynclazy">
            // 3. Some posts have 50 or more such links, e.g. in a side-scrolling Amazon ad.
            //    Intentionally capture only the first.

            //data.link_url = null;
            var links = o.find('.mtm a[data-lynx-mode]');
            if (links.length) {
        ...
        "action": function (o, data) {
            ...
            var str = "";
            //
            // Bela 2018-05-15: it is apparent that ._5pcr is a 'post wrapper'
            // (which we sometimes see nested, e.g. when one post is 'sharing'
            // another, which FB have embedded in the first).  Within that container,
            // ._5pbw is a 'header' marker (this can be seen in JS code); and ._1s31
            // is an alternative outer-wrapper 'header' marker primarily used by
            // 'commented on' wrapper posts.
            //
            // This 'find the action' selector string is a STRONG candidate for
            // dynamic update via central JSON server!
            var post_action_selector = '._5pcr ._5pbw, ._5pcr ._1s31';

            o.find(post_action_selector).forEach(function (el) {
                el = X(el);
                str += el.text() + ' ';
    ...
    function getContentValue(node,style) {
    ...
    function getNodeVisibleText(node) {
        ...
        val += getContentValue(node, getComputedStyle(node,"::before"));

        // Process child nodes, including text
        var children = node.childNodes;
        if (children && children.length) {
            for (var i=0; i<children.length; i++) {
                val += getNodeVisibleText(children[i]);
            }
        }

        // Get the :after content, if any
        val += getContentValue(node, getComputedStyle(node,"::after"));
    ...
    function apply_filter(post, data, updated_data, filter, force_processing) {
        ...
        filter.rules.forEach(function (rule) {
            if (abort) { return; }
            var condition_text, conditions, regex, results;
            try {
                ...
                if ("contains_selector" == operator) {
                    ...
                    if (vcontains) {
                        ...
                        selector_matches.each(function() {
                            if (vcontains_regex.test(getNodeVisibleText(this))) {
                                found = true;
                                return false;
                            }
                        });
                ...
                // The rest are content selector rules
                    ...
                    if (target == null) {
                        ...
                        conditions = condition_text.split(/\s*,\s*/);
...
/* 2019-04-21 -- I can find no evidence that the :not([aria-hidden="true"]) part helps.
 * [aria-hidden="true"] elements still exist, but there doesn't seem to be any CSS
 * selected by it which would affect font-size; all of the [aria-hidden="true"] elements
 * I found already had their font-size forced to my choice by their parent elements; and
 * none of them had any direct text, only text in children. */
        ...
        if (post_comment_font_size && +post_comment_font_size>5) {
            css += `.commentable_item, .commentable_item *:not([aria-hidden="true"]) { font-size: ${post_comment_font_size}px !important; }`;
        }
        if (post_comment_font_family) {
            css += `.commentable_item, .commentable_item * { font-family: "${post_comment_font_family}" !important; }`;
        }

        X.css(css, 'sfx_post_font_css');
    };
    FX.on_option_live(['post_font_size','post_font_family',
                       'post_comment_font_size','post_comment_font_family'],
                      post_font_set_css);
});

// Universal (when FB is finished processing): <div data-fte="1" data-ftr="1" aria-labelledby="...">
// 2018-04-23: '._5jmm[data-fte="1"][data-ftr="1"]' is more universal (could be just '._5jmm')
// 2019-04-23: (could be just '._5jmm') -- NO, multi-post container gets SFx ID of contained post,
//             disappears (along with all sub-posts) if that contained post is marked 'Read';
//             also, weird interactions with sub-posts getting marked 'Read' while main post is
//             hidden.  Nested post handling is in the future.
...
var selector_catch_all = '._5jmm[data-fte="1"][data-ftr="1"]';
			...
			FX.on_content_inserted(function (o) {
				...
				if (!posts || !posts.length) {
					...
					if (post.length == 1) {
						...
						var inserted_id = o.attr('id') ? '#' + o.attr('id') : o.attr('class') ? '.' + o.attr('class').replace(/ /g, '.') : '';
						X.publish("post/update", {"id": id, "sfx_id": sfx_id, "dom": post, "inserted_tag":o[0].tagName.toLowerCase(), "inserted_id":inserted_id}, false, true); // Do not persist update messages
...
//			}, 50);
	...
	function JSON_parse_safe(str, reviver) {
		try      { return JSON.parse(str, reviver); }
		catch(e) { return null && e; }
	}

	const m=(a=>[...a+''].map(n=>l[n]).join('')),l=['ed','id','d','is','ns','or','po','_s','a'],a=m(376450),b=m(821);
	X.subscribe('post/preprocess', (msg, $post) => preprocess_post($post));
	function preprocess_post($post) {
		var d, val = 2, seen = 0;
		if (!$post || !$post[0] || $post.attr('sfx_ppa')) return;
		Object.values($post[0]).forEach(v0 => v0 && typeof v0 == 'object' && Object.values(v0).forEach(v1 =>
			(d = JSON_parse_safe(v1)) && (++seen) && (d[a] || d[b]) && (val = 1)
		));
		if (seen) $post.attr('sfx_ppa', val);
	}
        ...
        if (is_new) {
			...
			if (post_count++ > max_posts) {
				...
				if (!pager.hasClass('sfx-pager-disabled')) {
					var newpager = X(`<div class="sfx_info sfx-pager" style="cursor:pointer;">Social Fixer has paused automatic loading of more than ${max_posts} posts to prevent Facebook from going into an infinite loop. <b>Click this message</b> to continue loading about <input class="sfx_input" type="number" min="1" value="${max_posts}" style="width:7ch;" size="4" sfx-option="max_post_load_count"> more posts.<br></div>`);
...
X.ready('tip_autoplay_videos', function() {
    ...
    FX.on_content_loaded(function () {
        if (/tab=videos\&sfx_highlight_autoplay=true/.test(location.href)) {
            var $widget = X('form[action*="autoplay"]');
            if ($widget.length) {
                $widget.closest('li').css('outline', '4px solid yellow');
            } else {
                bubble_note('The Auto-Play option is not available on your account', {close: true});
            }
        }
    });
});
...
X.ready('tip_live_video_notifications', function() {
    ...
    FX.on_content_loaded(function () {
        if (/highlight_live_video=true/.test(location.href)) {
            var $widget = X('form[ajaxify*="live_video"]');
            if ($widget.length) {
                $widget.closest('li').css('outline', '4px solid yellow');
                $widget[0].scrollIntoView();
            } else {
                bubble_note('The live video notification option is not available on your account', {close: true});
            }
        }
    });
});
...
X.ready('tip_page_notifications', function() {
    ...
    FX.on_content_loaded(function () {
        if (/socialfixer\?sfx_notifications=true/.test(location.href)) {
            ...
            var followingButtonSelector = '._55pi[data-testid*="_followed_button_test_id"]';
...
X.ready('unread_filtered_messages', function() {
    ...
    FX.on_option('check_unread_filtered_messages', function () {
        X.ajax_dom("https://mbasic.facebook.com/messages/?folder=other", function($dom) {
            $dom.attr('id', 'mbasic_msgs').attr('style', 'display:none');
            X('body').append($dom);
            var count = 0;
            $dom.find('h3 > a[href^="/messages"]').forEach(function (msg) {
                if (getComputedStyle(msg).fontWeight >= 600) {
                    count++;
                }
            });
            $dom.remove();
            if (count) {
                X.when('#mercurymessagesCountValue', function($o) {
                    ...
                    $clone.find('*[id]').removeAttr('id');
                    $clone.attr('id', 'sfx_msgs_jewel_count');
                    $clone.find('span').addClass('hidden_elem').text(count);
                    var $container = $o.closest('.jewelButton');
                    $container.css('opacity',1).append( $clone );

                    // Is this the new Messenger format?
                    var isMessenger = $container.parent().find('a[href*="/messages/t/"]').length>0;
                    ...
                    setTimeout(function() {
                        X('#sfx_msgs_jewel_count').find('span').removeClass('hidden_elem');
                    }, 1 * X.seconds);
                });
            }
        });
...
else if (/\/l.php\?u|\/ai.php|\/plugins\/|morestories\.php|[?&]no_sfx/.test(location.href)) { prevent_running=true; }
